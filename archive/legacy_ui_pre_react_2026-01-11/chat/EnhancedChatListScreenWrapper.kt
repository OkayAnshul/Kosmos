...
/**
 * EntryPoint for accessing repositories in Compose
 */
@EntryPoint
@InstallIn(SingletonComponent::class)
interface ChatListDependenciesEntryPoint {
...
fun EnhancedChatListScreenWrapper(
    ...
    val dependencies = remember {
        EntryPointAccessors.fromApplication(
            ...
            ChatListDependenciesEntryPoint::class.java
        )
    }
    val chatRepository = remember { dependencies.chatRepository() }
    val authRepository = remember { dependencies.authRepository() }
    val currentUserId = remember { authRepository.getCurrentUser()?.id ?: "" }
    ...
    val unreadCounts = uiState.chatRooms.associate { chatRoom ->
        chatRoom.id to chatRepository.getUnreadCountFlow(chatRoom.id, currentUserId)
            .collectAsState(initial = 0).value
    }

    // Convert ChatRoom list to ChatRoomItem list
    val chatRoomItems = remember(uiState.chatRooms, unreadCounts) {
        uiState.chatRooms.map { chatRoom ->
            chatRoom.toChatRoomItem(
                unreadCount = unreadCounts[chatRoom.id] ?: 0,
                isPinned = chatRoom.isPinned,
                isOnline = false // TODO: Implement online status via presence system (Supabase Realtime)
            )
        }
    }
