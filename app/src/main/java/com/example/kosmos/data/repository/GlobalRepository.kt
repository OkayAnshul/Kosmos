...
class ChatRepository @Inject constructor(
    ...
    fun getChatRoomByIdFlow(chatRoomId: String): Flow<ChatRoom?> = flow {
        // Emit local data first
        val localChatRoom = chatRoomDao.getChatRoomByIdFlow(chatRoomId)
        localChatRoom.collect { chatRoom ->
            emit(chatRoom)
        }
    ...
    suspend fun markMessageAsRead(messageId: String, userId: String) {
        try {
            ...
            if (message != null) {
                ...
                val updates = mapOf("readBy" to updatedReadBy)
    ...
    suspend fun loadOlderMessages(chatRoomId: String, beforeTimestamp: Long, limit: Int = 20): List<Message> {
        return try {
